...
**Supabase とは？** クラウド上の PostgreSQL データベースです。無料枠があり、サーバー管理が不要です。**東京リージョン**が選べるため、日本からのアクセスが高速です。
...
2. 「Start your project」→ GitHub アカウントでログイン（おすすめ）
3. ダッシュボードが表示されれば成功
...
1. 「New Project」をクリック
2. 以下を入力:
   - **Organization:** 自分の Organization を選択（初回は自動で作成されています）
   - **Project name:** `uketori`
   - **Database Password:** 安全なパスワードを入力 → **必ずメモしてください**（後で接続文字列に使います）
     - できれば英数字と `-` `_` を中心にすると接続文字列へ埋め込む時に安全です
   - **Region:** `Northeast Asia (Tokyo)` ← **日本リージョンを選択**
3. 「Create new project」をクリック
4. プロジェクトの初期化に1〜2分かかります

---

### 3-3. 接続情報をメモ

プロジェクト作成後、接続情報を取得します。

1. プロジェクト画面右上の **「Connect」** をクリック
2. 「Connection string」セクションを開く
3. **「Session pooler」** を選択（ポート `5432`）
4. 表示されている接続文字列を**すべてコピー**してメモ

   例: `postgresql://postgres.[project-ref]:[YOUR-PASSWORD]@aws-0-ap-northeast-1.pooler.supabase.com:5432/postgres`

5. `[YOUR-PASSWORD]` の部分を Step 3-2 で設定したデータベースパスワードに置き換えてください
6. この文字列が後で使う `DATABASE_URL` です

> **重要:** この文字列にはパスワードが含まれています。他人に見せないでください。
>
> **パスワードに記号を使った場合:** `@`、`:`、`/`、`?`、`#` などを含むと URL として壊れることがあります。その場合は URL エンコードするか、英数字中心のパスワードで作り直してください。
>
> **なぜ Session pooler なのか？** Supabase 公式では、常駐するバックエンドサーバーには `Direct connection`、IPv4 環境ではその代替として `Session pooler` を推奨しています。AWS Lightsail は IPv4 前提で使うことが多いため、この手順書では `Session pooler (5432)` を採用します。
...
# --- データベース ---
DATABASE_URL=ここにSupabaseの接続文字列を貼る
...
> Supabase では Step 3 でコピーした **Session pooler（ポート 5432）** の接続文字列を使っているか確認してください。
...
**なぜ？** データベースが壊れたり、誤操作でデータを削除してしまった場合に復元するためです。Supabase は日次バックアップを自動で取得しますが、無料運用では**自分でもバックアップを持っておく**前提にしておくと安全です。

---

### 10-1. Supabase のバックアップ画面を確認

Supabase は無料プランでも日次バックアップを自動取得します。ただし公式にも Free tier では CLI / `pg_dump` による定期エクスポートが推奨されています。無料運用では「自前バックアップ」を前提に考えてください。

1. https://supabase.com/dashboard にログイン
2. プロジェクト `uketori` を選択
3. 左メニュー「Database」を開く
4. 接続情報と稼働状況が確認できれば OK

> **Supabase の機能だけで大丈夫？** 日次バックアップだけでは、欲しい時点に細かく戻せない場合があります。月末に先月分のデータ消失へ気づくようなケースにも備えて、次の手順で定期バックアップを設定します。
...
| `DATABASE_URL` | `postgresql://postgres.[project-ref]:[password]@aws-0-ap-northeast-1.pooler.supabase.com:5432/postgres` | Step 5-7 (Step 3 で取得) |
...
Phase 3: 成長期             Lightsail Small ($12)  ¥1,800/月
    ↓ 200ユーザー超 or DB容量逼迫
Phase 4: 拡大期             Lightsail Medium ($24) + Supabase Pro ($25)  ¥7,500/月
    ↓ 500ユーザー超 or 可用性要件
...
### Phase 3→4: Small ($12) → Medium ($24) + Supabase Pro

**タイミング:** 200ユーザー超、または以下のいずれかに該当:
- Supabase Free のデータベース容量上限（500MB）に近づいている
- 接続数や運用機能の制約が本番運用のボトルネックになっている
...
#### 手順 B: Supabase を Pro プランにアップグレード
...
|------|------|-------------|
| データベース容量 | 500MB | 8GB |
| 日次バックアップ | 自動取得あり（自前バックアップ推奨） | 7日分へアクセス可能 |
| Point-in-Time Recovery | 利用不可 | 別料金アドオンで利用可能 |
| 本番向け運用機能 | 最小限 | バックアップ運用を強化しやすい |

> **補足:** Supabase Free でも MVP は十分動きます。Pro へ上げる主目的は、容量拡大に加えて、バックアップ運用を強化しやすくすることです。PITR は必要になった時だけ別料金アドオンとして検討してください。
...
- 高可用性（ダウンタイム最小化）が必要
- オートスケーリングが必要
- Supabase でも DB をアプリと同じ AWS アカウント/VPC 内に置きたくなった
...
**2. Supabase → RDS にデータを移行**

```bash
# Lightsail サーバーで実行

# 1. Supabase からデータをダンプ
docker compose -f docker-compose.production.yml exec -T api \
  ...
  > /home/ubuntu/supabase_dump.sql

# 2. RDS にデータをリストア（RDSのエンドポイント・パスワードに置き換え）
sudo apt install -y postgresql-client
psql "postgres://postgres:パスワード@RDSエンドポイント:5432/uketori_production" < /home/ubuntu/supabase_dump.sql
```
