...
export default function EditProjectPage() {
  ...
  const loadProject = useCallback(async () => {
    try {
      ...
      reset({
        ...
        assigned_user_id: p.assigned_user_id ?? undefined,
  ...
  useEffect(() => {
    ...
    loadUsers();
    loadProject();
  }, [loadCustomers, loadUsers, loadProject]);
  ...
  const onSubmit = async (data: ProjectFormData) => {
    try {
      await api.patch(`/api/v1/projects/${uuid}`, {
        project: data,
      });
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" asChild className="size-10 sm:size-9">
          <Link href={`/projects/${uuid}`}>
            <ArrowLeft className="size-5 sm:size-4" />
          </Link>
        </Button>
        <div>
          <h1 className="text-2xl font-bold tracking-tight">案件の編集</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Button variant="outline" type="button" asChild>
            <Link href={`/projects/${uuid}`}>キャンセル</Link>
          </Button>
